...
void show_menu(const char *app_name)
{
	printf("********************** usage ********************\n");
	printf("      server\n");
...
void server(char *addr, short port)
{
	...
	/* 2.bind the server to the ip address */
	if(bind(fd, (struct sockaddr *)&sin, sizeof(sin)) == -1)
	{
		perror("bind");
		exit(-1);
	}
	printf("******** bind server address succcess *******\n");
	...
	printf("******** server starting succcess ***********\n");
...
void client(char *addr, short port)
{
	...
	printf("******** client connected succcess ***********\n");
