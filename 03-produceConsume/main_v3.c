...
sem_t consume_1to2, consume_2to1, produce_1to2, produce_2to1;
...
void init(struct prodcons *b){
	...
	sem_init(&produce_1to2, 0, 1);
	sem_init(&produce_2to1, 0, 0);
...
void *producer1(void *data){
	...
	gettimeofday(&current_time, NULL);
        printf("producer1-----: ");
	printf("put red to buffer (%ld)\n", current_time.tv_usec);
	buffer.buffer[buffer.writepos]=current_time.tv_usec;
...
void *consumer1(void *data){
	...
	if(d > 0)
		printf("get data from buffer is red(%ld)\n", d);
	else
		printf("get data from buffer is blue(%ld)\n", d);
